import { register, login, generateToken } from './authService'
...
describe('register', () => {
  ...
  it('throws 409 if email is already registered', async () => {
    ...
    await expect(register('test@example.com', 'password123', 'Test User')).rejects.toMatchObject({
      statusCode: 409,
...
describe('login', () => {
  ...
  it.each([
    ...
    jest.mocked(prisma.user.findUnique).mockResolvedValue(user as typeof mockUser | null)
...
describe('generateToken', () => {
  ...
  it('throws if JWT_SECRET is not configured', () => {
    ...
    expect(() => generateToken('user-id' as any, 'test@example.com')).toThrow('JWT_SECRET not configured')
  })

  it('signs the token with userId and email in the payload', () => {
    jest.mocked(jwt.sign).mockReturnValue('signed-token' as never)
    generateToken('user-id' as any, 'test@example.com')
