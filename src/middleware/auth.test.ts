...
describe('authMiddleware', () => {
  ...
  it('calls next() and attaches userId to req for a valid token', () => {
    jest.mocked(jwt.verify).mockReturnValue({ userId: 'user-uuid', email: 'test@example.com' } as never)
  ...
  it.each([
    ['expired token', new jwt.TokenExpiredError('expired', new Date()), 'Token expired'],
    ['tampered token', new jwt.JsonWebTokenError('invalid'), 'Invalid token'],
  ])('returns 401 for %s', (_, error, expectedMessage) => {
    jest.mocked(jwt.verify).mockImplementation(() => { throw error })
    const res = makeRes()
    const next = jest.fn()
    ...
    expect(res.status).toHaveBeenCalledWith(401)
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: expectedMessage }))
    expect(next).not.toHaveBeenCalled()
  })
})
